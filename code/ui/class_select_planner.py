...
class SelectPlanner(QWidget, Ui_select_planner):
    ...
    def set_schedule_item_list(self):        # 선택 일에 맞춰 변경되도록 설정하기(아직 대기)
        ...
        self.viewer = self.listWidget_select_list  # 선택 목록 리스트위젯 인스턴스화
        layout.addWidget(self.viewer)
        self.setLayout(layout)

        for i in range(10):
            item = QListWidgetItem(self.viewer)
            custom_widget = LocationItem(f"{i + 1}일차 일정", "밑으로 옮겨주세요", "구분")
    ...
    def set_location_item_lsit(self):       # 주소 리스트위젯화
        layout = QBoxLayout(QBoxLayout.TopToBottom)
        self.viewer_2 = self.listWidget_rec_location  # 선택 목록 리스트위젯 인스턴스화
        layout.addWidget(self.viewer_2)
        self.setLayout(layout)

        for idx, item_ in enumerate(self.location_list):
            item = QListWidgetItem(self.viewer_2)
            custom_widget = LocationItem(f"{item_[0]}",
                                         f"{item_[1]}",
                                         f"{item_[2]}")
            item.setSizeHint(custom_widget.sizeHint())  # item에 custom_widget 사이즈 알려주기
